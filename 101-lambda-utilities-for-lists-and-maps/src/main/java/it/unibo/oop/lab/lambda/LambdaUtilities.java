...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
         ...
         */
        final List<Optional<T>> passedOpts = new ArrayList<>();
        list.forEach(t -> passedOpts.add(Optional.of(t).filter(pre::test)));
        return passedOpts;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
         ...
         */
        final Map<R, Set<T>> passedElems = new HashMap<>();
        list.forEach(t -> passedElems.merge(op.apply(t), Set.of(t), (x, y) -> {
            final Set<T> z = new HashSet<>(x);
            z.add(t);
            return z;
        }));
        return passedElems;
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
         ...
         */
        final Map<K, V> passedElems = new HashMap<>();
        map.forEach((k, v) -> passedElems.put(k, v.orElse(def.get())));
        return passedElems;
    }
